...
export function createUserData (filePath) {
  ...
  function userDoc (userId) {
    ...
    if (!Array.isArray(doc.profiles) || !doc.profiles.length) {
      doc.profiles = [{ id: DEFAULT_PROFILE_ID, name: 'Principal', kids: false, createdAt: new Date().toISOString() }]
    }
  ...
  return {
    ...
    addProfile (userId, { id, name, kids } = {}) {
      const doc = userDoc(userId)
      ...
      const profile = { id: pid || crypto.randomUUID().slice(0, 8), name: clean, kids: !!kids, createdAt: new Date().toISOString() }
