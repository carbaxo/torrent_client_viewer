...
export function createCatalog ({ fetchImpl, tmdbKey, cache, region = 'ES', timeoutMs = 10000 } = {}) {
  ...
  async function getCatalog (platformKey, mediaType = 'movie') {
    if (!tmdbKey) {
    ...
    const key = `${platformKey}:${mediaType}:${region}`
  ...
  async function getAllCatalogs (mediaType = 'movie') {
    const settled = await Promise.allSettled(PLATFORMS.map((p) => getCatalog(p.key, mediaType)))
    const catalogs = []
    const warnings = []
    settled.forEach((r, i) => {
  ...
  return { getCatalog, getAllCatalogs, platforms: PLATFORMS }
}
